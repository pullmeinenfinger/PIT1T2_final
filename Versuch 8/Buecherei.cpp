...
int main()
{
    ...
    do  /// Anzeige des Menüs
    {
        std::cout << std::endl
				  ...
				  << "(6): Alle ausgeliehene Medien ausgeben" << endl
                  << "(7): Beenden" << std::endl;
    ...
    /// Beim Beenden des Programms sämtliche Medienlöschen
...
void fuelleDatenbank(std::vector<Medium*>& medien)
{
#if UNTERKLASSENVORHANDEN
    {
        ...
        neuesMedium = new Magazin("RWTH MAG", Datum(), "Science");
		medien.push_back(neuesMedium);
    }
#else
    {
...
void mediumAusleihen(std::vector<Medium*>& medien, Datum aktuellesDatum)
{
    ...
    for(std::vector<Medium*>::iterator it = medien.begin(); it != medien.end(); it++)
    {
        if((*it)->getID() == id)
        {
            ...
            if ((*it)->getStatus() == false) {(*it)->ausleihen(person,aktuellesDatum);}
            else {std::cout << "Medium ist schon ausgelieht!\n";}
        }
    }
    if (!id_vorhanden)
    {
...
void mediumZurueckgeben(std::vector<Medium*>& medien)
{
    ...
    for(std::vector<Medium*>::iterator it = medien.begin(); it != medien.end(); it++)
    {
        ...
        {
            id_vorhanden = true;
            if((*it)->getStatus() == true){(*it)->zurueckgeben();}
            else {std::cout << "Medium ist nicht ausgelieht!\n";}
        }
    }
    if (!id_vorhanden)
    {
...
void alleMedienAusgeben(std::vector<Medium*>& medien)
{
    ...
    {
        std::cout << "*************************************************************" << std::endl;
        std::cout << *medien[index]; ///Operatorüberladung
    }
}
